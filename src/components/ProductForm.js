...
const ProductForm = () => {
  ...
  const generatePDFWithBarcodes = (product) => {
    ...
    for (let i = 0; i < product.quantity; i++) {
      ...
      // New page if full
      if (
        count % (labelsPerRow * rowsPerPage) === 0 &&
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/api/products`,
        { name, quantity, price, ...form, vendor: vendorId  },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setForm({
        name: "",
  ...
  return (
    <div className="container mt-4">
      ...
      <form
        ...
        className="p-4 border rounded-4 shadow-lg bg-white mb-5"
      >
        <h5 className="mb-3 pb-2 border-bottom text-dark fw-semibold">
          Product Details
        </h5>
